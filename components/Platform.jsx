...
function DocumentViewer({ fileUrl, fileName, type, onTextSelect }) {
  ...
  const loadDocx = async () => {
    ...
    try {
      ...
      const { value, messages } = await window.mammoth.convertToHtml({ arrayBuffer: buf });
  ...
  if (loading) return (
    <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',background:'#f8fafc'}}>
      <div style={{textAlign:'center',color:'#94a3b8'}}>
        ...
        <p style={{fontSize:13,fontWeight:600,marginTop:8}}>Loading document…</p>
      </div>
    </div>
  );
  ...
  if (type === 'PDF Document') return (
    <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
      <div style={{background:'#f8fafc',borderBottom:'1px solid #e2e8f0',padding:'6px 14px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'32px 40px',background:'#fff'}} onMouseUp={handleMouseUp}>
        ...
        {docHtml
          ...
          : <div style={{textAlign:'center',color:'#94a3b8',paddingTop:60}}><Spinner/><p style={{marginTop:12}}>Rendering…</p></div>
        }
      </div>
    </div>
  );
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      {sheets.length > 1 && (
        <div style={{display:'flex',gap:4,padding:'8px 12px',background:'#f8fafc',borderBottom:'1px solid #e2e8f0',flexShrink:0,overflowX:'auto'}}>
          ...
          <span style={{marginLeft:'auto',fontSize:11,color:'#94a3b8',alignSelf:'center',whiteSpace:'nowrap'}}>📊 {fileName}</span>
        </div>
      )}
      <div style={{flex:1,overflow:'auto',padding:16,background:'#fff'}} onMouseUp={handleMouseUp}>
        ...
        {sheets.length > 0
          ...
          : <div style={{textAlign:'center',color:'#94a3b8',paddingTop:60}}><Spinner/><p style={{marginTop:12}}>Parsing spreadsheet…</p></div>
        }
      </div>
    </div>
  );
  ...
  if (type?.includes('PowerPoint') || type?.includes('pptx')) {
    ...
    return (
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <iframe src={officeUrl} style={{flex:1,width:'100%',border:'none'}} title="PowerPoint Viewer"
          onError={()=>setViewErr('Could not load PowerPoint viewer. Please download the file to review.')}/>
      </div>
    );
  }
  ...
  if (type === 'Audio File') return (
    <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#f0f9ff,#e0e7ff)',gap:20,padding:40}}>
      ...
      <div style={{textAlign:'center'}}>
        ...
        <p style={{fontSize:12,color:'#6366f1'}}>Use the timestamp field in the comment panel →</p>
      </div>
  ...
  // ── FALLBACK (Other) ──────────────────────────────────────────
  return (
    <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#f8fafc,#eff6ff)',padding:40}}>
