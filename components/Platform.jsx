...
// DOCUMENT VIEWER
// ══════════════════════════════════════════════════════════════
function DocumentViewer({ fileUrl, fileName, type }) {
  if (!fileUrl) return (
  ...
  if (type==='PDF Document') return <div style={{flex:1,overflow:'hidden'}}><iframe src={fileUrl} style={{width:'100%',height:'100%',border:'none'}} title="PDF"/></div>;
  if (type==='Audio File') return (
    <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#f0f9ff,#e0e7ff)',gap:20,padding:40}}>
      ...
      <audio controls src={fileUrl} style={{width:'100%',maxWidth:380,borderRadius:8}}/>
    </div>
  );
  ...
  return (
    <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',background:'linear-gradient(135deg,#f8fafc,#eff6ff)',padding:40}}>
      <div style={{textAlign:'center',maxWidth:280,background:'#fff',borderRadius:20,padding:36,boxShadow:'0 4px 20px rgba(0,0,0,0.06)',border:'1px solid #e2e8f0'}}>
        ...
        <p style={{fontSize:13,color:'#64748b',marginBottom:20}}>Download to review, then add annotations in the comments panel.</p>
...
// ANNOTATION PANEL
// ══════════════════════════════════════════════════════════════
function AnnotationPanel({ material, currentVersion, roleId, user, onAdd, onResolve }) {
    ...
    <div style={{display:'flex',flexDirection:'column',height:'calc(100vh - 108px)'}}>
      ...
      <div style={{flex:1,display:'flex',overflow:'hidden'}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <div style={{flex:1,display:'flex',overflow:'hidden'}}>
            <DocumentViewer fileUrl={curV?.file_url} fileName={curV?.file_name} type={mat.type}/>
            <AnnotationPanel material={mat} currentVersion={curV} roleId={roleId} user={user} onAdd={onAddAnn} onResolve={onResolveAnn}/>
          </div>
